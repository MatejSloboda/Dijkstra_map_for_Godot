...
func recalculate_dijkstra_maps():
	...
	var dragon_position_id = point_position_to_id.get(world_to_map(get_node("dragon").position), 0)
	...
	var optional_parameters = {"terrain_weights": speed_modifiers, "input_is_destination": true}
...
func _unhandled_input(event):
	if event is InputEventMouseButton:
		...
		var dragon_position_id = get_node("dragon")
