...
func _ready():
	...
	pos_to_id=dijkstramap.add_square_grid(1,bmp,0,1.0,INF)
...
func recalculate():	
	...
	dijkstramap.recalculate(target_ids,{"terrain weights": terrain_weights})
	...
	for id in  costs.keys():
		...
		costgrid.set_cell(id_to_pos[id].x,id_to_pos[id].y,0,false,false,false,Vector2(cost,0))
	
	var dir_to_tile={Vector2(1,0):0,Vector2(1,-1):1,Vector2(0,1):2,Vector2(1,1):3,
		Vector2(-1,0):4,Vector2(-1,1):5,Vector2(0,-1):6,Vector2(-1,-1):7}
...
func _unhandled_input(event):
	...
	if (event is InputEventMouseMotion or
		event is InputEventMouseButton) and dragging:
