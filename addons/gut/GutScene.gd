...
onready var _progress = {script = $ScriptProgress, test = $TestProgress}
onready var _summary = {failing = $Summary/Failing, passing = $Summary/Passing}
...
onready var _titlebar = {bar = $TitleBar, time = $TitleBar/Time, label = $TitleBar/Title}
...
func _on_FocusBlocker_gui_input(ev):
	if _text_box_blocker_enabled:
		...
		# convert a drag into a pan gesture so it scrolls.
			...
			and (ev.button_index == BUTTON_WHEEL_DOWN or ev.button_index == BUTTON_WHEEL_UP)
		):
			get_text_box()._gui_input(ev)
	else:
		get_text_box()._gui_input(ev)
